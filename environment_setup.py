...
class environement_setup:
    ...
    def install_env(self):
        ...
        if python_version[0] != 3:
            print("Warning: Python version must be 3.7.x to work properly. Errors may rise!")

        try:
            requirement_file = sys.argv[1]
        except:
            print('You did not pass a conda environment file. Do you want to install the non conda environment? [Y/n]?')
            x = input()
