...
class stripanalysis:
    ...
    def find_pinhole(self, Idiel, shift=None, suppress_warning=False):
        ...
        if len(highIdiel):
            ...
            if not suppress_warning:
                self.log.warning(
                    "Possible pinholes found on strips: {}".format(highIdiel)
                )
        else:
    ...
    def find_bad_DC_contact(
        self, Istrip, Rpoly, Cint, Cap, shift=None, suppress_warning=False
    ):
    ...
    def find_bad_AC_contact(
        self, Cap, Rpar, pinholes, shift=None, suppress_warning=False
    ):
        ...
        if len(intersect):
            ...
            if not suppress_warning:
                self.log.warning(
                    "Possible bad AC needle contact found on strips: {}".format(
                        intersect
                    )
                )
        else:
        ...
        for data in self.all_data:
            if not self.all_data[data]["analysed"]:
                ...
                if len(LowCap):
                    ...
                    self.find_metal_and_implant_open(
                        LowCap,
                        working_data,
                        piecewiselms,
                        cutted_array,
...
if __name__ == "__main__":
    det = stripanalysis(
        None,
        "C:\\Users\\dbloech\\PycharmProjects\\Doktorat\\QTC-Software\\COMET\\COMET\\config\\config\\badstrip.yml",
    )
