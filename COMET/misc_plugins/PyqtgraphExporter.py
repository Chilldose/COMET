...
"""This is a temporary exporter, since the pyqtgraph exporter is broken. And has not been fixed in over a year
If you experience problems with the exporter try to remove him and use the original exporter"""
...
class PQG_ImageExporter(Exporter):
    ...
    def export(self, fileName=None, toBytes=False, copy=False):
        ...
        bg = np.empty(
            (int(self.params["width"]), int(self.params["height"]), 4), dtype=np.ubyte
        )
        ...
        try:
            ...
            self.getScene().render(
                painter, QtCore.QRectF(targetRect), QtCore.QRectF(sourceRect)
            )
        finally:
