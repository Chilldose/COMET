...
# Generate lists that are used later on to store differents parameter values for all the files used in the analysis
#for analysis mos
cv = []
fbvoltage = []
...
class IV_PQC:
    def __init__(self, data, configs):
        #Start the analysis from the gate diode files if available
        if not all(i[0:6] == 'IV_GCD' for i in list(data.keys())):
            i = 0
            while list(data.keys())[0][0:6] == 'IV_GCD':
                data[list(data.keys())[0] + str(i)] = data[list(data.keys())[0]] #push the gate diode files to the first elements of the list
        ...
        self.xaxis = self.measurements[0] #choose x axis, normally voltage.
    ...
    def run(self):
        ...
        for df in self.data["keys"]:
            ...
            if self.data[df]['header'][1][0:8] != 'CV_Diode' and self.data[df]['header'][1][0:6] != 'IV_GCD': #If it is not a Diode or a Gate file do the cv mos analysis.
        ...
        self.PlotDict["All"] = self.plot(self.PlotDict["BasePlots_diode"], self.PlotDict["BasePlots_MOS"], self.PlotDict["BasePlots_gate"], cv, diode, gate)
        return self.PlotDict


    def plot(self, diodePlots, mos_Plots, gate_Plots, cv, diode, gate):
        # Select the plots to show depending on the kind of files you have
        if len(cv) != 0 and len(diode) != 0 and len(gate) != 0:
            self.PlotDict['All'] = gate_Plots + diodePlots + mos_Plots
        elif len(cv) != 0 and len(diode) != 0 and len(gate) == 0:
    ...
    def analysis_gate(self,df):
        ...
        gate.append(df)  # append to a list containing all the gate diode files
        ...
        # generate curve
        points_wout_kink = (self.data[df]["data"][self.xaxis], CurrentCopy)
        plot_not_kink = hv.Curve(points_wout_kink, label='Capacitance')
        plot_not_kink = customize_plot(plot_not_kink, "Current", self.config["IV_PQC"])

        # Trial of savgol filter
        try:
            ...
            maxsavgol = max(y)
            points_savgol_filter = (self.data[df]['data']['Voltage'], y)
            curr_savgol_plot = hv.Curve(points_savgol_filter)
            curr_savgol_plot = customize_plot(curr_savgol_plot, "SavgolCurrent", self.config["IV_PQC"])
        except:
            print('Exception for the savgol plot')

        f = interp1d(self.data[df]["data"][self.xaxis], CurrentCopy, kind='cubic')  # f is the interpolation function
        ...
        points_interpolation = (xnew, ynew)
        curr_interp_plot = hv.Curve(points_interpolation).opts(title="Current_Gate")
        ...
        der_one_interp = diffy_interp / diffx_interp  # first derivative
        firstderi_interp = np.insert(der_one_interp, 0, der_one_interp[0])
        points_firstderi_interp = (xnew, firstderi_interp)
        dif_intep_plot = hv.Curve(points_firstderi_interp)
        dif_intep_plot = customize_plot(dif_intep_plot, "FirstDerivativeCurrent", self.config["IV_PQC"])

        # second derivative
        diffy_interp2 = np.diff(ynew, n=2)
        diffy_interp2_add = np.insert(diffy_interp2, 0,
                                      diffy_interp2[0])  # add an element to have the correct length of the array
        der_two_interp = diffy_interp2_add / diffx_interp
        second_deriv_interp = np.insert(der_two_interp, 0, der_two_interp[0])
        points_second_deriv_interp = (xnew, second_deriv_interp)
        dif2_intep_plot = hv.Curve(points_second_deriv_interp)
        dif2_intep_plot = customize_plot(dif2_intep_plot, "SecondDerivativeCurrent", self.config["IV_PQC"])

        # try to find the start and ending indeces of the points where you want to average
        ...
        dx1 = np.diff(self.data[df]["data"][
                          "Voltage"])  # compute the difference between each consecutive voltage value and store it into an array with length i-1
        der1 = dy1 / dx1
        firstdev_not_interp = np.insert(der1, 0,
                                        der1[0])  # Add an element to the array to have the same number of rows as in df

        # insert first derivative into dataframe
        self.data[df]["data"].insert(3, "firstderivative_gate", firstdev_not_interp)
        ...
        miny = np.mean(list(ynew[
                            -1000:]))  # find the minimum of the current-voltage curve by averaging 20 points values in the curve tail
        ...
        S_null_max = Isurf_max / (self.config['IV_PQC_parameter']['q'] * self.config['IV_PQC_parameter'][
            'n_i_intrinsic_carrier_concentration'] * (float(self.data[df]['header'][0].split(':')[1]) * (1e-8)))
        Surface_recombination_velocity.append(S_null_max)

        # compute Surface_recombination_velocity with the average method
        S_null_average = I_surf_average / (self.config['IV_PQC_parameter']['q'] * self.config['IV_PQC_parameter'][
            'n_i_intrinsic_carrier_concentration'] * (float(self.data[df]['header'][0].split(':')[1]) * (1e-8)))
        ...
        if len(gate) == 1:
            ...
            self.donts_gatediode = ["timestamp", "voltage", "Voltage", 'Current', "Stepsize", "Wait", "Stepsize",
                                    "Frequency", 'x', 'N']  # don't plot these.
            ...
            self.PlotDict[
                "BasePlots_gate"] += text * curr_interp_plot * plot_not_kink * Path_min * Path_mxx * Path_average * Path_Isurf * Path_Isurf_average * Path_savgolmax
            ...
            df4 = pd.DataFrame({"Name": gate, "Surface current_max (A)": Surface_current,
        ...
        # do this if the analysis is of more than one file
            ...
            self.donts = ["timestamp", "voltage", "Voltage", "Stepsize", "Wait", "Stepsize", "Frequency", 'x',
                          'N']  # do not plot this data
    ...
    def analysis_diode(self,df):
        ...
        diode.append(df)  # Append to a list containing all the diode files
        self.data[df]["data"]["Voltage"] = list(
            map(abs, self.data[df]["data"]["Voltage"]))  # take absolute value of Voltage

        ####try interpolation + filtered savitzy-golay derivative plot
        # interpolate the capacity
        f = interp1d(self.data[df]["data"][self.xaxis], self.data[df]["data"]["Capacity"],
                     kind='cubic')  # f is the interpolating function
        xnew = np.arange(self.data[df]['data']['Voltage'][0], list(self.data[df]['data']['Voltage'][-1:])[0], 0.001)
        ynew = f(xnew)
        ...
        points_capacity = (xnew, Onec2)  # points to use for the plot
        ...
        # savgolay plot
        derivative_savgol_filtered = scipy.signal.savgol_filter(add_element_derivative, 5,
                                                                2)  # window size 51, polynomial order 3
        points_derivative_savgol_filtered = (xnew, derivative_savgol_filtered)
        deronec2_savgol_plot = hv.Curve(points_derivative_savgol_filtered)
        ...
        firstdev_invers_C2 = np.insert(der_invers_C2, 0, der_invers_C2[
        ...
        # Plot all Measurements
        self.donts_diode = ["timestamp", "voltage", "Voltage", "Stepsize", "Wait", "Stepsize", "Frequency", 'x', 'N',
                            'Capacity', 'Current']  # do not plot capacity voltage plot
        ...
        self.basePlots_2 = plot_all_measurements(self.data, self.config, 'x', self.name,
                                                 do_not_plot=['Voltage', 'Current', 'Capacity', '1C2',
                                                              'derivative1C2', 'x'],
                                                 keys=diode)  # diode is the list containing all the diode files
        ...
        d_active = self.config['IV_PQC_parameter']['epsilonNull'] * self.config['IV_PQC_parameter'][
            'epsilonSiliconOxide'] * (float(self.data[df]['header'][0].split(':')[1]) * (1e-8)) * (
                       1e-2) / C_min  # in cm
        T_n = 295 / 300
        u_holes_mobility = 54.3 * pow(T_n, -0.57) + 1.36 * (1e+8) * pow(295, -2.23) / (
                    1 + ((5e+12) / (2.35 * (1e+17) * pow(T_n, 2.4))) * 0.88 * pow(T_n, -0.146))  # in cm^2/(V*s)
    ...
    def analysis_mos(self,df):
        cv.append(df)  # Append to a list containing all the cv mos files that you want to analyze

        if 'Voltage' in self.data[df]['data']:  # Check that the voltage values have increasing order
            if self.data[df]["data"]["Voltage"][0] > 0:
                self.data[df]["data"]["Voltage"] = list(
                    reversed(self.data[df]["data"]["Voltage"]))  # if decreasing order, reverse it.
        ...
        self.data[df]["data"]["Capacity"] = self.data[df]["data"]["Capacity"] / (
                ...
                1e-8))  # Normalize by the Area and convert to cm^2
        CapacityCopy = self.data[df]["data"]["Capacity"].copy()
        capMin = np.max(self.data[df]["data"]["Capacity"][
                        :20])  # Find the Maximum among the first 20 values of the Capacity and set it as the minimum Capacity value
        ...
        # Do interpolation
        f = interp1d(self.data[df]["data"][self.xaxis], self.data[df]["data"]["CapacityCopy"],
                     kind='cubic')  # f is the interpolation function
        xnew = np.arange(self.data[df]['data']['Voltage'][0], list(self.data[df]['data']['Voltage'][-1:])[0], 0.001)
        ynew = f(xnew)
        points_capacitance_interpolation = (xnew, ynew)
        capa_interp_plot = hv.Curve(points_capacitance_interpolation)
        ...
        # Compute derivatives of the interpolated data
        ...
        points_interpolated_derivative = (xnew, firstdev1_interp)
        derivative_interpolation_plot = hv.Curve(points_interpolated_derivative)
        derivative_interpolation_plot = customize_plot(derivative_interpolation_plot, "derivative",
                                                       self.config["IV_PQC"])
        points_interpolated_sec_derivative = (xnew, seconddev_interp)
        secondderivative_interp_plot = hv.Curve(points_interpolated_sec_derivative)
        secondderivative_interp_plot = customize_plot(secondderivative_interp_plot, "derivative2",
                                                      self.config["IV_PQC"])

        # Find the flatbandvoltage by means of the maximum value of the first derivative
        ele_max = firstdev1_interp.argmax()
        voltage_value_of_max_element = xnew[ele_max]
        maxderiplot = hv.VLine(voltage_value_of_max_element).opts(line_width=1.0)
        fbvoltage_firstderivative.append(voltage_value_of_max_element)
        ...
        # Plot all Measurements
        self.donts_mos = ["timestamp", "voltage", "Voltage", "Stepsize", "Wait", "Stepsize", "Frequency", 'x', 'N',
                          'Current']  # don't plot these.
        self.basePlots5 = plot_all_measurements(self.data, self.config, self.xaxis, self.name,
                                                do_not_plot=self.donts_mos,
                                                keys=cv)  # keys= cv, where cv is the list cointaing all the cv mos file to analyze
        ...
        if self.config["IV_PQC"].get("CapacityCopy", {}).get("findFlatBandVoltage", False):
            try:
                ...
                fBestimation = self.find_flatBand_voltage(clone_plot, self.data, self.config, indexMax, indexMin, df,
                                                          cv, voltage_value_of_max_element,
                                                          PlotLabel="Flat band voltage estimation")

                # Append the values resulting from the analysis to the corresponding list.
        ...
        if len(cv) == 1:
            ...
            self.PlotDict[
                "BasePlots_MOS"] += capa_interp_plot * maxderiplot * derivative_interpolation_plot * secondderivative_interp_plot
        ...
        df2 = pd.DataFrame(
            {"Name": cv, "Flatband Voltage second_derivative (V)": fbvoltage,
             'Flatband Voltage first_derivative (V)': fbvoltage_firstderivative,
             'Accumulation capacitance (F)': Accum_capacitance,
             'Accumulation capacitance normalized (F/cm^2)': Accum_capacitance_normalized, 'Tox (nm)': Tox,
             'Nox (cm^-2)': Nox})
    ...
    def find_flatBand_voltage(self, plot, data, configs, indexMax, indexMin, df, cv, voltage_value_of_max_element, **addConfigs,): #cv is the list containing all the cvmos files
        """
        ...
        text = hv.Text(10, 0.00000000065, 'Flat band voltage_fit_2nd derivative: {} V \n'
            ...
            np.round(voltage_value_of_max_element, 2),
    ...
    def find_full_depletion_c2(self, plot, data, configs,diode, **addConfigs):
        ...
        self.log.info('Full depletion voltage: {} V'
                        .format(np.round(full_depletion_voltages[0], 2)))
