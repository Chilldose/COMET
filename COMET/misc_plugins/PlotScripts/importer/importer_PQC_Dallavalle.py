...
area = {}
return_dict = {}
all_files_types_of_analysis = (
    []
)  # List that store the kind of analysis that need to be done for your files
...
def importer(filepaths, **kwargs):
    ...
    for fileidx, file in enumerate(filepaths):
        ...
        file_name = os.path.basename(
            filepaths[fileidx]
        )  # Take the last element of the file path as the name of the file
        dic[file_name] = file_content[fileidx].split(
            "\n"
        )  # Assign the file name to its content in a dcitionary
        dic[file_name] = [
            i.split("â")[0] for i in dic[file_name]
        ]  # Sometimes for certain data there is an additional â that here is removed
        dic[file_name] = list(
            filter(None, dic[file_name])
        )  # Remove blank lines from the data
        analysis_type = [s for s in dic[file_name] if "Run" in s][0].split()[
            0
        ]  # Define the analysis type (gate, diode or mos)
        all_files_types_of_analysis.append(analysis_type)
        index_of_datastart = (
            dic[file_name].index([s for s in dic[file_name] if "Run" in s][0]) + 4
        )  # (3+1) from Run entry to data values +3, +1 because the first data value is often unstable (ignore it)
        ...
        data_extracted = extract_data(
            file_name, dic[file_name], index_of_datastart, end_data
        )
...
def return_dictionary(data_extracted, file_name, analysis_type):
    if len(data_extracted[2]) > 1:  # For files containing capacity measurements
        return_dict[file_name] = {
            "data": {
                "Voltage": data_extracted[0],
                "Current": data_extracted[1],
                "Capacity": data_extracted[2],
            }
        }
        return_dict[file_name]["header"] = [
            data_extracted[3],
            analysis_type,
            all_files_types_of_analysis,
        ]
        return_dict[file_name].update(
            {"measurements": ["Voltage", "Current", "Capacity"]}
        )
        return_dict[file_name].update({"units": ["V", "A", "F"]})
    else:
        return_dict[file_name] = {
            "data": {"Voltage": data_extracted[0], "Current": data_extracted[1]}
        }
        return_dict[file_name]["header"] = [
            data_extracted[3],
            analysis_type,
...
def extract_data(file_name, dictionary, index_of_datastart, end_data):
    ...
    try:
        ...
        log.warning(
            "Not all the voltage data is a float number, check that you have written it correctly... Error: {}".format(
                err
            )
        )
    try:
        ...
        log.warning(
            "Not all the current data is a float number, check that you have written it correctly... Error: {}".format(
                err
            )
        )
    try:
        ...
        log.warning(
            "Impossible to extract capacity values, check if you wrote it correctly or if it is contained in the file..."
        )
