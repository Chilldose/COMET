...
class IVCV_class(tools):
    ...
    def run(self):
        ...
        try:
            ...
            self.log.error(
                "Could not perform open correction for IVCV measuremnt routine, due to missing LCR meter device."
            )
        time = self.do_IVCV()
    ...
    def do_IVCV(self):
        ...
        if "IV" in self.main.job_details["IVCV"]:  # Creates the actual measurement plan
            ...
            voltage_End.append(
                self.main.job_details["IVCV"]["IV"].get("End Voltage [V]", 0)
            )
            voltage_Start.append(
                self.main.job_details["IVCV"]["IV"].get("Start Voltage [V]", 0)
            )
            voltage_steps.append(
                self.main.job_details["IVCV"]["IV"].get("Voltage Step [V]", 0)
            )
        ...
        if "CV" in self.main.job_details["IVCV"]:
            ...
            voltage_End.append(
                self.main.job_details["IVCV"]["CV"].get("End Voltage [V]", 0)
            )
            voltage_Start.append(
                self.main.job_details["IVCV"]["CV"].get("Start Voltage [V]", 0)
            )
            voltage_steps.append(
                self.main.job_details["IVCV"]["CV"].get("Voltage Step [V]", 0)
            )
        ...
        if self.main.save_data:
            ...
            units_to_write = []
            units_to_write.append(
                self.main.job_details["IVCV"]["Units"].get("X-Axis", [])
            )
            ...
            self.main.write(
                self.main.measurement_files["IVCV"], units_str + "\n"
        ...
        if self.main.job_details["IVCV"].get("Voltage Steps Refinement", False):
            voltage_step_list = self.refine_ramp(
                voltage_step_list,
                self.main.job_details["IVCV"]["Voltage Steps Refinement"][
                    "Start Voltage[V]"
                ],
                self.main.job_details["IVCV"]["Voltage Steps Refinement"][
                    "End Voltage[V]"
        ...
        compliance = (
            str(
                self.main.job_details["IVCV"].get(
                    "IV", self.main.job_details["IVCV"].get("CV", None)
                )["Compliance [uA]"]
            )
            + "e-6"
        )
        ...
        if self.IVCV_configs["Meas_order"] == "interlaced":
            i = self.interlaced_order(
                voltage_step_list, bias_SMU, LCR_meter, compliance
            )
        elif self.IVCV_configs["Meas_order"] == "sequential":
            i = self.sequential_order(
                voltage_step_list, bias_SMU, LCR_meter, compliance
            )
        else:
            self.log.error(
                "Measurement order: {} was recognised, either sequential or interlaced are possible".format(
                    self.IVCV_configs["Meas_order"]
                )
            )
        ...
        self.do_ramp_value(
            bias_SMU,
            "set_voltage",
            self.main.settings["settings"]["bias_voltage"],
            0,
            20,
            1.0,
            set_value=self.change_bias_voltage,
        )
        ...
        if self.check_compliance(bias_SMU, 100e-6):
            ...
            if self.check_compliance(bias_SMU, 100e-6):
                self.log.error(
                    "Output voltage was set to 0 and still the device is in compliance. Please check the setup."
    ...
    def sequential_order(self, voltage_step_list, bias_SMU, LCR_meter, compliance):
                ...
                for i, voltage in enumerate(voltage_step_list):
                    ...
                    if self.main.job_details["environment"]:
                        env_array.append(
                            ...
                            + str(self.main.event_loop.humidity_history[-1]).ljust(
                                self.justlength
                            )
                        )
                    if abs(voltage) <= abs(
                        self.main.job_details["IVCV"][meas].get("End Voltage [V]", 0)
                    ):
                        iter = i
                        if (
                            ...
                            getattr(self, "do_{}".format(meas))(
                                voltage, device, samples=self.samples
                            )

                        elif (
                            self.main.event_loop.stop_all_measurements_query()
                        ):  # Stops the measurement if necessary
                            break
        ...
        if self.main.save_data:
            try:
                if (
                    not self.main.measurement_data["IV"][1][0].any()
                    or not self.main.measurement_data["CV"][1][0].any()
                ):  # if the array was not yet populated add one otherwise diff is one short (empty array has len 1!)
                    shift = 1
                else:
                    shift = 0
                diff = (
                    len(self.main.measurement_data["IV"][1])
                    - len(self.main.measurement_data["CV"][1])
                    + shift
                )
                if diff > 0 and len(self.main.measurement_data["CV"][1]) > 0:
                    self.main.measurement_data["CV"][0] = np.append(
                        self.main.measurement_data["CV"][0],
                        self.main.measurement_data["IV"][0][
                            len(self.main.measurement_data["CV"][0]) :
                        ],
                    )
                    self.main.measurement_data["CV"][1] = np.append(
                        self.main.measurement_data["CV"][1],
                        [np.nan for x in range(diff)],
                    )

                    self.main.measurement_data["CVQValue"][0] = np.append(
                        self.main.measurement_data["CVQValue"][0],
                        self.main.measurement_data["IV"][0][
                            len(self.main.measurement_data["CVQValue"][0]) :
                        ],
                    )
                    self.main.measurement_data["CVQValue"][1] = np.append(
                        self.main.measurement_data["CVQValue"][1],
                        [np.nan for x in range(diff)],
                    )

                elif diff < 0 and len(self.main.measurement_data["IV"][1]) > 0:
                    self.main.measurement_data["IV"][0] = np.append(
                        self.main.measurement_data["IV"][0],
                        self.main.measurement_data["CV"][0][
                            len(self.main.measurement_data["IV"][0]) :
                        ],
                    )
                    self.main.measurement_data["IV"][1] = np.append(
                        self.main.measurement_data["IV"][1],
                        [np.nan for x in range(diff)],
                    )
                ...
                for entry in range(len(self.main.measurement_data["IV"][1])):
                    ...
                    if "IV" in self.main.job_details["IVCV"]:
                        string_to_write += str(
                            self.main.measurement_data["IV"][0][entry]
                        ).ljust(self.justlength)
                    elif "CV" in self.main.job_details["IVCV"]:
                        string_to_write += str(
                            self.main.measurement_data["CV"][0][entry]
                        ).ljust(self.justlength)

                    # Now add the actuall value
                    if (
                        "IV" in self.main.job_details["IVCV"]
                        and self.main.measurement_data["IV"]
                    ):
                        string_to_write += str(
                            self.main.measurement_data["IV"][1][entry]
                        ).ljust(self.justlength)
                    if (
                        "CV" in self.main.job_details["IVCV"]
                        and self.main.measurement_data["CV"]
                    ):
                        string_to_write += str(
                    ...
                    self.main.write(
                        self.main.measurement_files["IVCV"], string_to_write + "\n"
                    )
            except AttributeError as err:
                self.log.critical(
                    "Attribute error happened during sequential data output to file. Err: {}".format(
                        err
                    )
                )
            except IndexError as err:
                self.log.critical(
                    "Indexing error while writing to file. Error: {}".format(err)
                )
    ...
    def interlaced_order(self, voltage_step_list, bias_SMU, LCR_meter, compliance):
        ...
        for i, voltage in enumerate(voltage_step_list):
            ...
            # Change the progress
            self.main.settings["settings"]["progress"] = (i + 1.0) / len(
                voltage_step_list
            )
            if (
                not self.main.event_loop.stop_all_measurements_query()
                ...
                string_to_write = str(self.main.measurement_data["IV"][0][-1]).ljust(
                    self.justlength
                )  # The voltage
                if "IV" in self.main.job_details["IVCV"] and abs(voltage) <= abs(
                    self.main.job_details["IVCV"]["IV"].get("End Voltage [V]", 0)
                ):
                    self.do_IV(voltage, bias_SMU, samples=self.samples)
                    if self.main.save_data:
                        string_to_write += str(
                            self.main.measurement_data["IV"][1][-1]
                        ).ljust(
                            self.justlength
                        )  # the current

                if "CV" in self.main.job_details["IVCV"] and abs(voltage) <= abs(
                    self.main.job_details["IVCV"]["CV"].get("End Voltage [V]", 0)
                ):
                    self.do_CV(voltage, LCR_meter, samples=self.samples)
                    if self.main.save_data:
                        string_to_write += str(
                            self.main.measurement_data["CV"][1][-1]
                        ).ljust(
                            self.justlength
                        )  # the capacitance
                ...
                if self.main.job_details.get("environment", False):
                    if "CV" in self.main.job_details["IVCV"] and abs(voltage) > abs(
                        ...
                        string_to_write += "--".ljust(self.justlength) + "--".ljust(
                            self.justlength
    ...
    def do_IV(self, voltage, device_dict, samples=5):
        ...
        if not self.main.event_loop.stop_all_measurements_query():
            ...
            if float(voltage) != 0.0 and not self.steady_state_check(
                device_dict,
                self.IVCV_configs["GetReadSMU"],
                max_slope=maxslope,
                wait=0.05,
                samples=5,
                Rsq=0.8,
                compliance=self.main.job_details["IVCV"]["IV"]["Compliance [uA]"],
                iteration=5,
            ):  # Is a dynamic waiting time for the measuremnt
                # self.stop_everything()
                self.log.info(
                    "Steady state could not be reached in LCR Meter, measurement may be compromised at voltage step = {}".format(
                        voltage
                    )
                )
                # return
            ...
            for i in range(samples):
                values.append(
                    str(
                        self.vcw.query(
                            device_dict, device_dict[self.IVCV_configs["GetReadSMU"]]
                        )
                    ).split("\t")
                )  # 2657SMU
            ...
            self.main.settings["settings"][
                "bias_voltage"
            ] = voltage  # changes the bias voltage
            self.biascurrent = float(values[0])

            self.main.measurement_data["IV"][0] = np.append(
                self.main.measurement_data["IV"][0], [float(voltage)]
            )
            self.main.measurement_data["IV"][1] = np.append(
                self.main.measurement_data["IV"][1], [float(values[0])]
            )
    ...
    def do_CV(self, voltage, device_dict, samples=5):
        ...
        if not self.main.event_loop.stop_all_measurements_query():
            ...
            if float(voltage) != 0.0 and not self.steady_state_check(
                self.main.devices[self.IVCV_configs["BiasSMU"]],
                self.IVCV_configs["GetReadSMU"],
                max_slope=maxslope,
                wait=0.05,
                samples=5,
            ...
            for i in range(samples):
                values.append(
                    str(
                        self.vcw.query(
                            device_dict, device_dict[self.IVCV_configs["GetReadLCR"]]
                        )
                    ).split(",")
                )
            values = np.array(values, dtype=float)
            ...
            self.main.measurement_data["CV"][0] = np.append(
                self.main.measurement_data["CV"][0], float(voltage)
            )
            self.main.measurement_data["CV"][1] = np.append(
                self.main.measurement_data["CV"][1], values[0]
            )
            # Save the QValue/second value as well
            self.main.measurement_data["CVQValue"][0] = np.append(
                self.main.measurement_data["CVQValue"][0], float(voltage)
            )
            self.main.measurement_data["CVQValue"][1] = np.append(
                self.main.measurement_data["CVQValue"][1], values[1]
            )
    ...
    def perform_open_correction(self, LCR, measurement="None"):
        ...
        self.main.queue_to_main.put(
            {"INFO": "LCR open calibration on {} path...".format(measurement)}
        )
        ...
        while True:
            ...
            if done:
                ...
                if counter > 10:
                    self.log.error(
                        "LCR meter did not answer after 10 times during open correction calibration."
                    )
                    self.stop_everything()
                counter += 1
        self.switching.switch_to_measurement("IV")
        self.main.queue_to_main.put(
            {"INFO": "LCR open calibration on {} path finished.".format(measurement)}
        )
