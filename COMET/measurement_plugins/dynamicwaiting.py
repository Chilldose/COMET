...
class Dynamicwaiting_class(tools):
    def __init__(self, main_class):
        ...
        self.interval = self.main.job_details["Dynamicwaiting"]["Interval"] / 1000.0
        ...
        if "2470 Keithley SMU" in self.biasSMU["Device_name"]:
            ...
            self.get_timestamps_query = (
                "printbuffer(1, {samples!s}, measbuffer.relativetimestamps)"
            )
            ...
            self.get_data_query = "printbuffer(1, {samples!s}, measbuffer)"
            self.get_timestamps_query = (
                "printbuffer(1, {samples!s}, measbuffer.timestamps)"
            )
            self.SMU_clean_buffer = "measbuffer = nil"
            self.SMUON = "1"
            self.SMUOFF = "0"
            ...
            self.setup_config = [
                ("set_compliance_current", str(self.compliance)),
                ("set_NPLC", "{!s}".format(self.NPLC)),
    ...
    def do_dynamic_waiting(self):
        ...
        for i, voltage in enumerate(self.voltage_step_list):
            if (
                ...
                self.main.settings["settings"]["progress"] = (
                    voltage / self.voltage_step_list[-1]
                )
                ...
                self.xvalues[i], self.yvalues[i], time = self.do_dynamic_measurement(
                    "Dynamicwaiting",
                    self.biasSMU,
                    voltage,
                    self.buffer,
                ...
                if self.check_compliance(
                    self.biasSMU, float(self.compliance), command="get_read",
                ):
        ...
        self.current_voltage = self.framework["Configs"]["config"]["settings"][
            "bias_voltage"
        ]
        self.do_ramp_value(
            self.biasSMU, "set_voltage", self.current_voltage, 0, 20, 0.01
        )
        ...
        self.write_dyn_to_file(
            self.main.measurement_files["Dynamicwaiting"],
            self.voltage_step_list,
            self.xvalues,
            self.yvalues,
        )
        ...
        if len(xvalues) == len(yvalues):
            ...
            self.main.write(
                ...
                "".join(
                    [
                        format(
                            "time[s]{}current[A]".format(" " * (self.justlength - 7)),
    ...
    def do_preparations(self, device, samples=100, interval=0.01):
        ...
        self.voltage_step_list = self.ramp_value(
            voltage_Start, voltage_End, voltage_steps
        )
        ...
        self.send_to_device(
            self.biasSMU,
            self.SMU_config.format(samples=samples, interval=interval, level=0),
            self.separate,
        )

    def do_dynamic_measurement(
        self,
        str_name,
        device,
        voltage=0,
        ...
        starttime = time()
        self.send_to_device(
            device, self.measureItobuffer.format(level=voltage, samples=samples)
        )
        ...
        while not device_ansered:
            ans = self.vcw.query(
                device, self.get_data_query.format(samples=samples)
            ).strip()
            times = self.vcw.query(
                device, self.get_timestamps_query.format(samples=samples)
            ).strip()
            if ans:
                device_ansered = True
            elif iter > 5:
        ...
        if ans:
            ...
            if (
                len(xvalues) > samples
            ):  # because some devices add an extra value to the end, and some dont. Good work again keithley
            ...
            if write_to_main:  # Writes data to the main, or not
                self.main.queue_to_main.put(
                    {str(str_name): [np.array(xvalues), np.array(yvalues)]}
                )
            # Clear buffer
            ...
            self.log.error(
                "Timeout occured while reading from the device! Increase timeout for devices if necessary"
