...
class ErrorMessageBoxHandler:
    ...
    def showdialog(self):
        ...
        self.msg.setText(
            "COMET encounterd {} error(s)".format(self.message_counter).ljust(70)
        )
    ...
    if exctype is not KeyboardInterrupt:
        ...
        QtWidgets.QMessageBox.critical(
            ...
            tr(
                "QMessageBox",
                "Exception type: {}\n" "Exception value: {}\n" "Traceback: {}",
            ).format(exctype.__name__, value, message),
        )
    # Pass on exception
...
class newThread(
    QtCore.QThread
):  # This class inherite the functions of the threading class
    ...
    def __init__(
        self, threadID, name, object__, *args
    ):  # Init where the threadID and Thread Name are defined
        """
        ...
        self.log.info(
            "Initialized new thread with ID: {!s}, Name: {!s} and Object: {!s}".format(
...
class table_control_class:
    ...
    def __init__(self, main_variables, devices, queue_to_GUI, vcw):
        ...
        if not "height_movement" in self.variables:
            ...
            self.variables["height_movement"] = 3000
        ...
        if "Table_control" in self.devices:
            if self.devices["Table_control"].get("Visa_Resource", None):
                ...
                self.log.error(
                    "Table control could not be initialized! Visa Resource missing"
                )
        else:
            self.table_ready = False
    ...
    def move_table_to_edge(self, axis, minimum=True, lifting=800, **kwargs):
        ...
        if var in self.devices["Table_control"]:
            ...
            if axis == "x":
                ...
                self.log.error(
                    "Table position recognition error. Table pos could not be determined correctly"
                )
        ...
        while cal_not_done:
            ...
            if done:
                try:
                    ...
                    self.log.error(
                        "Table status query failed to interpret: {} must be a float convertible".format(
                            done
                        ),
                        exc_info=True,
                    )
            ...
            elif done == 1.0:  # motors are in movement
                self.vcw.write(self.device, ready_command)  # writes the command again

            elif done % 2 == 1.0:
                self.vcw.write(self.device, ready_command)  # writes the command again
                self.log.critical(
                    "Table status has reported several status messages besides table moving. Additional Status: {}".format(
                        (done - 1.0)
                    )
                )
                done = done - 1.0

            if done == 2.0:  # joystick active
                self.log.error("Joystick of table control is active.")
                return False

            elif done == 4.0:  # joystick active
                ...
                self.log.error(
                    "The table control reported an unknown error, with errorcode: "
                    + str(done)
            ...
            elif done == 0.0:  # when corvus is read again
                return True
    ...
    def initiate_table(self):
        ...
        if self.table_ready and not self.variables["table_is_moving"]:
            ...
            self.log.error(
                "An error occured while trying to initiate the table. This can happen if either no "
    ...
    def move_to_strip(
        self, pad_file, strip, transfomation_class, T, V0, height_movement, **kwargs
    ):
        """
        ...
        if transformation != []:
            if not self.__already_there(
                pad_file, strip, transfomation_class, T, V0, skip=False
            ):  # This skip must alwasy be false, only set it to true if you know what you doin
                pad_pos = pad_file["data"][str(strip)]
                self.log.info(
                    "Moving to strip: {} at position {},{},{}.".format(
                        strip, pad_pos[0], pad_pos[1], pad_pos[2]
                    )
                )
                table_abs_pos = list(transfomation_class.vector_trans(pad_pos, T, V0))
                success = self.move_to(
                    table_abs_pos, move_down=True, lifting=height_movement, **kwargs
                )
            else:
                return True
...
class switching_control:
    ...
    def __init__(self, settings, devices, queue_to_main, vcw):
        ...
        for dev in self.devices.values():
            if (
                "Switching relay" in dev.get("Device_type", "None")
                and "Visa_Resource" in dev
            ):
    ...
    def apply_specific_switching(self, switching_dict):
        ...
        for device in self.devices:
            if device in switching_dict.keys():
                found_any = True
                if not self.change_switching(
                    self.devices[device], switching_dict[device]
                ):
                    self.log.error(
                        "Manual switching was not possible for device {}".format(device)
                    )
                    return False
        if not found_any:
            self.log.error(
                "Could not find switching device: {}. It may not be connected or specified.".format(
                    device
                )
            )
            return False
    ...
    def manual_switching(self, device, configs, BBM=True):
        ...
        current_switching = str(
            self.vcw.query(device, command)
        ).strip()  # Get current switching
        current_switching = self.pick_switch_response(device, current_switching)

        to_open_channels = list(
            set(current_switching) - (set(configs))
        )  # all channels which need to be closed
    ...
    def check_all_closed_channel(self, device, to_be):
        ...
        while device_not_ready:
            ...
            if all_done == opc_success:
                current_switching = str(
                    self.vcw.query(device, all_closed_command)
                ).strip()
                current_switching = self.pick_switch_response(device, current_switching)
                self.settings["settings"]["current_switching"][
                    device["Device_name"]
                ...
                if (
                    len(command_diff) != 0
                ):  # Checks if all the right channels are closed
                    self.log.error(
                        "Switching to {}  was not possible. Difference read:{}".format(
                            to_be, current_switching
                        )
                    )
                    return False
                return True
...
class show_cursor_position:
    ...
    def __init__(self, plotobject):
        ...
        self.proxy = pg.SignalProxy(
            plotobject.scene().sigMouseMoved, rateLimit=30, slot=self.onMove
        )
        self.log = logging.getLogger(__name__)
    ...
    def onMove(self, pos):
        ...
        if mousePoint:
            self.tooltip_text.setText(
                "     x={!s}\n     y={!s}".format(
                    int(round(mousePoint.x(), 0)), EngUnit(mousePoint.y())
                )
            )
...
def reset_devices(devices_dict, vcw):
    """Reset all devices."""
    l.critical(
        "You are using a depricated reset devices function please remove it from your code"
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--reinit",
        help="Calls the init window to initialize the setups",
        action="store_true",
    )
    parser.add_argument(
        "--noGUI", help="If the GUI should be displayed or not", action="store_true"
    )
    parser.add_argument(
        "--fullscreen", help="Shows the app in fullscreen mode", action="store_true"
    )
    parser.add_argument("--loadGUI", help="Load a specific GUI", type=str)
    parser.add_argument(
        "--minimal",
        help="Loads only the bare minimum for headless applications.",
        action="store_true",
    )
    parser.add_argument(
        "--update",
        help="Checks for updates in the requirement files and updats them if need be.",
        action="store_true",
    )
...
def convert_to_df(to_convert, abs=False):
    ...
    for key, data in to_convert.items():
        ...
        try:
            ...
            data["data"]["Name"] = [
                key for i in range(len(data["data"][list(data["data"].keys())[0]]))
            ]
...
def save_dict_as_hdf5(data, dirr, base_name):
    ...
    for key in df.get("keys", []):
        data[key]["data"].to_hdf(
            os.path.join(dirr, "singledata", "{}.hdf5".format(key)), key="df", mode="w"
        )
...
def send_telegram_message(person, message, configs, client):
    ...
    if not "Telegram" in client:
        l.debug(
            "Telegram message could not be dispatched since no TCP/IP client was specified!"
        )
        return False
    ...
    if "TelegramBot" in configs:
        if person in configs["TelegramBot"]:
            l.debug(
                "Operator found to send telegram message to. Operator: {}".format(
                    person
                )
            )
            send_TCP_message(
                client["Telegram"],
                "TelegramBot",
                {str(configs["TelegramBot"][person]): message},
            )
            return True
        else:
            l.debug(
                "No telegram ID defined for Operator: {}. No message send. Message: {}".format(
                    person, message
                )
            )
            return False
        ...
        l.debug(
            "Not TelegramBot entry in the configs, no message dispatch: Message: {}".format(
                message
            )
        )
        return False
