...
class measurement_job_generation:
    ...
    def generate_strip(self, header):
        ...
        all_measurements = [
            ...
            "Cback",
            "CintAC",
        ]  # warning if this is not included here no job will generated. is intentional
        ...
        if self.variables["Stripscan_measure"][0]:
            final_dict.update(
                {
                    "StartVolt": 0,
                    "EndVolt": self.variables["Stripscan_measure"][1],
                    "compliance": str(self.variables["Stripscan_measure"][2]) + "e-6",
                    "Steps": self.variables["Stripscan_measure"][3],
                }
            )
...
class Main_window:
    def __init__(self, GUI_classes, layout):
        ...
        self.layout = layout
        self.log = logging.getLogger(__name__)
        self.job = measurement_job_generation(
            self.variables.default_values_dict, self.variables.message_from_main
        )
        ...
        self.table_move_ui = self.variables.load_QtUi_file(
            "table_control.ui", self.table_widget
        )
    ...
    def table_move(self):
        ...
        self.layout.addWidget(
            self.table_widget,
            self.table_ypos + self.table_ysize + 1,
            self.table_xpos,
            1,
            1,
        )
    ...
    def IV_plot(self):
        ...
        self.layout.addWidget(
            iv_plot, self.IV_posy, self.IV_posx, self.IV_ysize, self.IV_ysize
        )
        ...
        def update(kwargs=None):
            ...
            if self.variables.default_values_dict["settings"]["new_data"]:
                if len(self.variables.meas_data["IV"][0]) == len(
                    self.variables.meas_data["IV"][1]
                ):  # sometimes it happens that the values are not yet ready
                    iv_plot.plot(
                        self.variables.meas_data["IV"][0],
                        self.variables.meas_data["IV"][1],
                        pen="y",
                        clear=True,
                    )
    ...
    def CV_plot(self):
        ...
        self.layout.addWidget(
            cv_plot, self.CV_posy, self.CV_posx, self.CV_ysize, self.CV_ysize
        )
        ...
        def update():
            if self.variables.default_values_dict["settings"]["new_data"]:
                if len(self.variables.meas_data["CV"][0]) == len(
                    self.variables.meas_data["CV"][1]
                ):  # sometimes it happens that the values are not yet ready
                    cv_plot.plot(
                        self.variables.meas_data["CV"][0],
                        map(depletion_volt, self.variables.meas_data["CV"][1]),
                        pen="y",
                        clear=True,
                    )
