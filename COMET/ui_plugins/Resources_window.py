...
class Resources_window:
    ...
    def __init__(self, GUI, layout):
        ...
        self.possible_states = {
            "CONNECTED": "QFrame { background :rgb(255, 215, 0) }",
            ...
            try:
                ...
                try:
                    if device_VISA_resource_name:
                        resource = self.variables.vcw.reconnect_to_device(
                            device_dict, device_VISA_resource_name
                    ...
                    self.log.error(
                        "Unknown error happened, during connection attempt to device: {} with error: {}".format(
                            device, err
                        )
                    )
            except Exception as err:
                self.log.error(
                    "Unknown error happened, during connection establishment attempt to device: {} with error: {}".format(
                        device, err
                    )
                )
    ...
    def test_connection_action(self, device):
        ...
        if self.variables.devices_dict[device].get("Visa_Resource"):
            success = self.variables.vcw.verify_ID(
                self.variables.devices_dict[device]["Visa_Resource"],
                self.variables.devices_dict[device].get("device_IDN_query", "*IDN?"),
            )
            if not success:
                self.log.error(
                    "Connection test failed with device {}. Please see logs to find error!".format(
                        device
                    )
                )
                ...
                self.variables.devices_dict[device]["State"] = "ERROR"
                self.log.error(
                    "Device IDN request did not match. Answer from device {} was not {}".format(
                        success, self.variables.devices_dict[device]["Device_IDN"]
                    )
                )
        else:
    ...
    def device_state(self, device, GUI):
        ...
        GUI.device_connected_label.setStyleSheet(
            self.possible_states.get(state, "None")
        )
    ...
    def update_device_states(self):
        ...
        for device, widget in self.list_of_instruments.items():
            if (
                not self.variables.devices_dict[device].get("State", "None")
                == widget.device_connected_label.text()
            ):
    ...
    def get_all_instruments(self):
        ...
        for device, value in self.variables.devices_dict.items():
            if (
                value not in already_in
            ):  # so that a multifunctional device is not rendered twice
                already_in.append(value)
                device_dict = self.variables.devices_dict[device]
                resources_widget = QWidget()

                instrument = self.variables.load_QtUi_file(
                    "Device_connection_info.ui", resources_widget
                )
                instrument.device_name_label.setText(device_dict["Device_name"])
                instrument.device_IDN_label.setText(
                    device_dict.get("Device_IDN", "None")
                )
                ...
                instrument.Test_pushButton.clicked.connect(
                    partial(self.test_connection_action, device)
                )
                instrument.Configure_pushButton.clicked.connect(
                    partial(self.configure_device_action, device)
                )
                instrument.Reconnect_pushButton.clicked.connect(
                    partial(self.reconnect_action, device)
                )
                instrument.Reset_pushButton.clicked.connect(
                    partial(self.reset_device_action, device)
                )
        ...
        if device_dict.get(
            ...
            self.log.info(
                "Configuring instrument: {!s}".format(
                    device_dict.get("Device_name", "NoName")
                )
            )
