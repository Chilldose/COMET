...
class Environement_widget(object):
    def __init__(self, gui):
        ...
        if not "Environment" in gui.child_layouts:
            self.envlog.error(
                "No layout found to render environment widget in. Skipping..."
            )
            return
        Env_Qwidget = QWidget()
        self.env_layout = gui.child_layouts["Environment"]
        self.Env_Widget = self.variables.load_QtUi_file(
            "environement_control.ui", Env_Qwidget
        )
        ...
        if (
            "temphum_update_interval"
            not in self.variables.default_values_dict["settings"]
        ):
        ...
        self.Env_Widget.temperature_bar.setStyleSheet(
            "QProgressBar::chunk{background-color: #cc1414;}"
        )
        self.Env_Widget.temperature_bar.setValue(self.Env_Widget.min_temp_spin.value())

        self.Env_Widget.humidity_bar.setStyleSheet(
            "QProgressBar::chunk{background-color: #2662e2;}"
        )
        ...
        if "temphum_controller" in self.variables.devices_dict:
            ...
            self.envlog.error(
                "No device found with name or alias 'temphum_controller'. No environement monitor can be started"
            )
    ...
    def __cut_arrays(self, data_array, maximum_time, arrays_to_cut):
        ...
        try:
            ...
            delta_time = (
                data_array[arrays_to_cut[0]][0][1] - data_array[arrays_to_cut[0]][0][0]
            )
    ...
    def update_temphum_plots(self):
        ...
        if self.variables.default_values_dict["settings"]["new_data"]:
            ...
            if (
                self.variables.meas_data["temperature_chuck"][1].any()
                and self.variables.meas_data["humidity"][1].any()
            ):
                ...
                # Set temp bar
                if (
                    temp <= self.Env_Widget.max_temp_spin.value()
                    and temp >= self.Env_Widget.min_temp_spin.value()
                ):
                    self.Env_Widget.temperature_bar.setValue(temp)
                elif temp > self.Env_Widget.max_temp_spin.value():
                    self.Env_Widget.temperature_bar.setValue(
                        self.Env_Widget.max_temp_spin.value()
                    )
                elif temp < self.Env_Widget.min_temp_spin.value():
                    self.Env_Widget.temperature_bar.setValue(
                        self.Env_Widget.min_temp_spin.value()
                    )

                # Set temp bar
                if (
                    hum <= self.Env_Widget.max_hum_spin.value()
                    and temp >= self.Env_Widget.min_hum_spin.value()
                ):
                    self.Env_Widget.humidity_bar.setValue(hum)
                elif hum > self.Env_Widget.max_hum_spin.value():
                    self.Env_Widget.humidity_bar.setValue(
                        self.Env_Widget.max_hum_spin.value()
                    )
                elif hum < self.Env_Widget.min_hum_spin.value():
                    self.Env_Widget.humidity_bar.setValue(
                        self.Env_Widget.min_hum_spin.value()
                    )
            ...
            ax.setTicks(
                [
                    get_thicks_for_timestamp_plot(
                        self.variables.meas_data["temperature_chuck"][0],
                        5,
            ...
            try:
                if len(self.variables.meas_data["temperature_chuck"][0]) == len(
                    self.variables.meas_data["humidity"][1]
                ):  # sometimes it happens that the values are not yet ready
                    p1.plot(
                        self.variables.meas_data["temperature_chuck"][0],
                        self.variables.meas_data["temperature_chuck"][1],
    ...
    def valuechange(self):
        ...
        self.variables.default_values_dict["settings"][
            "control_environment"
        ] = self.Env_Widget.env_control_check.isChecked()
        self.variables.default_values_dict["settings"][
            "Log_environment"
        ] = self.Env_Widget.log_env_check.isChecked()

        max = self.Env_Widget.max_hum_spin.value()
        min = self.Env_Widget.min_hum_spin.value()

        meanval = abs(max + min) / 2
        humcommand = build_command(
            self.variables.devices_dict["temphum_controller"], ("set_hum", meanval)
        )
        offcommand = build_command(
            self.variables.devices_dict["temphum_controller"],
            ("set_environement_control", "OFF"),
        )
        oncommand = build_command(
            self.variables.devices_dict["temphum_controller"],
            ("set_environement_control", "ON"),
        )

        self.variables.vcw.write(
            self.variables.devices_dict["temphum_controller"], offcommand
        )
        self.variables.vcw.write(
            self.variables.devices_dict["temphum_controller"], humcommand
        )

        if self.Env_Widget.env_control_check.isChecked():
            self.variables.vcw.write(
                self.variables.devices_dict["temphum_controller"], oncommand
            )
    ...
    def update_bars_and_spinboxes(self):
        ...
        self.Env_Widget.temperature_bar.setRange(
            self.Env_Widget.min_temp_spin.value(), self.Env_Widget.max_temp_spin.value()
        )
        self.Env_Widget.humidity_bar.setRange(
            self.Env_Widget.min_hum_spin.value(), self.Env_Widget.max_hum_spin.value()
        )
