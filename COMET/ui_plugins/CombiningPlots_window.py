...
class CombiningPlots_window:
    def __init__(self, GUI, layout):
        ...
        self.current_plot_object = (
            None  # This is a list of tuples (plot_obj, session, path_in_session)
        )
        self.plotting_thread = None
        self.to_combine_plots = (
            []
        )  # This is a list of tuples (plot_obj, session, path_in_session)
        ...
        self.widget.output_tree.itemClicked.connect(
            self.get_current_selected_plot_object
        )
        self.widget.combine_treeWidget.itemClicked.connect(
            self.get_current_selected_to_combine_plot_object
        )
        self.widget.add_plot_pushButton.clicked.connect(self.add_action)
        self.widget.session_comboBox.currentIndexChanged.connect(
    ...
    def combine_and_show_action(self):
        ...
        self.combinedPlotOptions = self.convert_config_to_dict(
            self.widget.templates_comboBox.currentText()
        )
        self.combinedPlotTemplate = self.widget.templates_comboBox.currentText()
        finalPlot = customize_plot(
            finalPlot,
            self.widget.templates_comboBox.currentText(),
            self.combinedPlotOptions,
        )
        save_plot("temp_combine_plot", finalPlot, path, save_as=["html"])
        self.widget.webEngineView.load(
            QUrl.fromLocalFile(os.path.join(path, "html", "temp_combine_plot.html"))
        )
    ...
    def session_change_action(self, text=None):
        ...
        if self.widget.session_comboBox.currentText():
            ...
            self.update_plt_tree(
                self.plotting_sessions[self.widget.session_comboBox.currentText()]
            )
    ...
    def refresh_action(self):
        ...
        try:
            ...
            self.plotting_sessions = self.variables.ui_plugins[
                "DataVisualization_window"
            ].plot_sessions
            ...
            self.log.error(
                "Could not load any plot sessions. It seems the plotting Tab is not rendered, or the plotting output is corrupted. Error: {}".format(
                    err
                )
            )
    ...
    def render_action(self):
        ...
        # Add the parameters
        template["Files"] = [
            self.widget.session_comboBox.itemText(i)
            for i in range(self.widget.session_comboBox.count())
        ]
        ...
        for analy in plotting_output.plotObjects:
            ...
            if isinstance(Allplots, hv.core.layout.Layout):
                try:
                    ...
                    self.log.error(
                        "An error happened during plot object access. Error: {}".format(
                            err
                        )
                    )
            else:
    ...
    def apply_option_button(self):
        ...
        self.combined_plot = relabelPlot(
            self.combined_plot, self.widget.plot_label_lineEdit.text()
        )
        ...
        try:
            ...
            if "PlotOptions" in self.combinedPlotOptions[self.combinedPlotTemplate]:
                self.combinedPlotOptions[self.combinedPlotTemplate][
                    "PlotOptions"
                ].update(
                    newItem
                )  # Kinda redundant
    ...
    def update_plot_options_tree(self, plot):
        ...
        for opt, value in (
            configs[self.combinedPlotTemplate].get("PlotOptions", {}).items()
        ):
