...
class EnvironmentMonitor_window:
    def __init__(self, GUI, layout):
        ...
        self.SettingsGui = self.variables.load_QtUi_file(
            "environment_monitor_settings.ui", self.Settingwidget
        )
        ...
        for room in self.rooms:
            ...
            self.roomsGui[room] = (
                self.variables.load_QtUi_file("environment_monitor.ui", widget),
                widget,
                TabWidget,
                Tablayout,
            )
        ...
        self.SettingsGui.env_updateInterval_slider.setValue(
            self.settings["temphum_update_interval"] / 1000.0
        )
        self.SettingsGui.env_history_slider.setRange(0, 200)
        self.SettingsGui.env_history_slider.setValue(
            self.settings.get("temp_history", 3600) / 3600.0
        )

        # Connect the slider to the update function
        self.SettingsGui.env_history_slider.sliderReleased.connect(self.valuechange)
        self.SettingsGui.env_updateInterval_slider.sliderReleased.connect(
            self.valuechange
        )
        ...
        self.SettingsGui.env_history_slider.valueChanged.connect(
            self.update_lcd_displays
        )
        self.SettingsGui.env_updateInterval_slider.valueChanged.connect(
            self.update_lcd_displays
        )
        ...
        self.SettingsGui.fade_green_slider.valueChanged.connect(
            self.update_lcd_displays
        )
        self.SettingsGui.fade_speed_slider.valueChanged.connect(
            self.update_lcd_displays
        )
    ...
    def update_env_control(self):
        ...
        if self.env_task:
            self.env_task.update_interval = float(
                self.SettingsGui.env_updateInterval_slider.value() * 1000
            )
