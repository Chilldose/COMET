...
class settings_widget(object):
    ...
    def get_all_settings(self):
        ...
        for setting in settings_list:
            settings_to_write = self.get_specific_settings_value(
                str(setting) + "_measure"
            )
    ...
    def get_specific_settings_value(self, data_storage):
        ...
        return self.variables.default_values_dict["settings"].get(
            str(data_storage), [False, 0, 0, 0]
        )

    def load_new_values(
        self, data_storage, checkbox, first_value, second_value, third_value
    ):
