...
class MeasurementConfig_window:
    def __init__(self, GUI, layout):
        ...
        self.settings = self.variables.framework_variables["Configs"]["config"].get(
            "MeasurementSettings", {}
        )
        self.ui_groups = {}
        self.settings_boxes = {}
        ...
        if not self.settings:
            self.log.info(
                "Measurement settings tab was loaded but no options have been given. Please add the settings to your project or load one"
            )
        ...
        self.SettingsGui = self.variables.load_QtUi_file(
            "SettingsTab.ui", self.SettingsMainWidget
        )
        self.layout.addWidget(self.SettingsMainWidget)

        if not self.settings:
            self.log.warning(
                "Could not load settings tab since no settings have been defined, please load one..."
            )
        self.construct_ui()

        self.SettingsGui.Unlock_pushButton.clicked[bool].connect(
            self.SettingsGui.scrollArea.setEnabled
        )
    ...
    def load_settings_from_file(self, path):
        ...
        if os.path.exists(os.path.normpath(path)):
            try:
                ...
                self.log.error(
                    "An error happened while loading yaml settings with error: {}".format(
                        err
                    )
                )
                return
            ...
            if self.settings[group]["Do"] and value["Do"]:
                ...
                for set, val in value.items():
                    if set != "Do":
                        job[Name][set] = val[
                            -1
                        ]  # Only the last value from all is the current set value in the spin box
            ...
            for groupName, group in self.settings.items():
                if isinstance(group, dict):
                    if "Measurements" in group:
                        ...
                        gui = self.variables.load_QtUi_file(
                            "SettingsFrame.ui", SettingsWidget
                        )
                        ...
                        try:
                            ...
                            self.log.error(
                                "An error happend during settings ui construction, this usually happens if you "
                                "have passed wrong options for the measruements. Error: {}".format(
                                    err
                                )
                            )
                        ...
                        self.log.error(
                            "Wrong typed settings options. Each group needs to have a 'Measurements' entry."
                        )
    ...
    def config_group(self, ui, Name, measurements):
        ...
        for measName, opt in measurements["Measurements"].items():
            MeasWidget = QWidget()
            conf_ui = self.variables.load_QtUi_file(
                "SettingsConfig_widget.ui", MeasWidget
            )
            ...
            for option, values in opt.items():
                if option != "Do":
                    ...
                    ui.EnableGroup_pushButton.clicked[bool].connect(
                        conf_ui.frame.setEnabled
                    )
                    ui.EnableGroup_pushButton.clicked[bool].connect(
                        partial(self.change_check, measurements)
                    )
                    conf_ui.enable_checkBox.clicked[bool].connect(
                        partial(
                            self.change_check, measurements["Measurements"][measName]
                        )
                    )
                    box.valueChanged.connect(
                        partial(
                            self.change_value,
                            box,
                            measurements["Measurements"][measName],
                            option,
                        )
                    )
