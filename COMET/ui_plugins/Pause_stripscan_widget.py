...
class pause_stripscan_widget(QDialog):
    def __init__(self, gui, parent=None):
        ...
        self.PauseWidget = self.gui.variables.load_QtUi_file(
            "Pause_widget.ui", self.Pause_Widget_object
        )
    ...
    def do_pause_action(self):
        ...
        while (
            not self.gui.variables.default_values_dict["settings"].get(
                "stripscan_is_paused", False
            )
            or counter >= 300
        ):  # Wait until the stripscan is paused
            counter += 1
            sleep(0.1)
            ...
            if self.stop:
                return
        if counter >= 300:
            self.PauseWidget.status_label.setText(
                "Stripscan script could NOT paused, no answer from the stripscan..."
            )
    ...
    def ramp_down_action(self):
        ...
        values = self.gui.variables.default_values_dict["settings"][
            "stripscan_device"
        ]  # (self.bias_SMU, "set_voltage", self.current_voltage, self.voltage_steps)
        self.tools.do_ramp_value(
            resource=values[0],
            order=values[1],
            voltage_Start=values[2],
            voltage_End=0.0,
            step=values[3],
            wait_time=0.5,
            compliance=0.0001,
    ...
    def ramp_previous(self):
        ...
        self.tools.do_ramp_value(
            resource=values[0],
            order=values[1],
            voltage_Start=self.gui.variables.default_values_dict["settings"][
                "bias_voltage"
            ],
            voltage_End=values[2],
            step=values[3],
            wait_time=0.5,
            compliance=0.0001,
            set_value=self.changeBiasVolt,
    ...
    def continue_action(self):
        ...
        while self.gui.variables.default_values_dict["settings"].get(
            "stripscan_is_paused", False
        ):  # Wait until the stripscan is paused
            sleep(0.1)
        self.gui.variables.default_values_dict["settings"][
            "stripscan_is_paused"
        ] = False
        self.stop = True
        self.close()
