...
class QTCSelfTest_window:
    ...
    def plot_config(self):
        ...
        self.widget.strip_plot_histogram.setTitle("Histogram on: No measurement selected", **self.titleStyle)
        ...
        if measurement:
            ...
            if ydata.any(): # Checks if data is available or if all is empty
                if len(xdata) == len(ydata):  # sometimes it happens that the values are not yet ready (fucking multithreading)
                    ...
                    self.widget.strip_plot.plot(xdata, ydata, pen="r", clear=True, width=8, connect="finite")
    ...
    def reconfig_plot(self, Title, ylabel, unit):
        ...
        self.widget.strip_plot.setLabel('bottom', "Number", units="#", **self.labelStyle)
        self.widget.strip_plot.setLabel('left', ylabel, units=unit, **self.labelStyle)

        self.widget.strip_plot_histogram.setTitle("Histogram on: " + str(Title), **self.titleStyle)
        self.widget.strip_plot_histogram.setLabel('left', "Count", units="#", **self.labelStyle)
        self.widget.strip_plot_histogram.setLabel('bottom', ylabel, units=unit, **self.labelStyle)
