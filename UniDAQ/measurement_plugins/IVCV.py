    ...
    def do_IV(self, voltage, device_dict, samples = 5):
        ...
        if not self.main.main.stop_measurement:
            ...
            if not self.switching.switch_to_measurement("IV"):
                self.stop_everything()
                return
            if not self.main.steady_state_check(device_dict, "get_read_current", max_slope=1e-6, wait=0, samples=4,Rsq=0.5, complience=self.main.job_details["IVCV"]["IV"]["Complience"]):  # Is a dynamic waiting time for the measuremnt
                self.stop_everything()
    ...
    def do_CV(self, voltage, device_dict, samples = 5):
        ...
        if not self.main.main.stop_measurement:
            ...
            if not self.switching.switch_to_measurement("CV"):
                self.stop_everything()
                return
            if not self.main.steady_state_check(device_dict, "get_read", max_slope=1e-6, wait=0.05, samples=3, Rsq=0.5, complience=None):  # Is a dynamic waiting time for the measuremnts
                self.stop_everything()
                self.log.warning("Steady state could not be reached, shutdown of measurement")
                return
            values = []
