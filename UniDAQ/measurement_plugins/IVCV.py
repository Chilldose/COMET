...
class IVCV_class(tools):
    ...
    def do_IVCV(self):
        ...
        self.config_setup(bias_SMU, [("set_compliance_current", compliance)])
        self.change_value(bias_SMU, "set_output", "1")
        ...
        for i, voltage in enumerate(voltage_step_list):
            if not self.main.event_loop.stop_all_measurements_query(): # To shut down if necessary
                ...
                if not self.steady_state_check(bias_SMU, "get_read_current", max_slope = 1e-6, wait = 0, samples = 5, Rsq = 0.5, complience=compliance): # Is a dynamic waiting time for the measuremnts
                    self.stop_everything()
        ...
        self.change_value(bias_SMU, "set_voltage", "0")
        sleep(2.)
        ...
        self.change_value(bias_SMU, "set_output", "0")
        if discharge_SMU:
            self.capacitor_discharge(discharge_SMU, discharge_switching, "set_terminal", "FRONT", do_anyway=True)
        return None
    ...
    def do_IV(self, voltage, device_dict, samples = 5):
        ...
        if not self.main.event_loop.stop_all_measurements_query():
            ...
            if not self.switching.switch_to_measurement("IV"):
                self.stop_everything()
                return
            if not self.steady_state_check(device_dict, "get_read_current", max_slope=1e-6, wait=0, samples=4,Rsq=0.5, complience=self.main.job_details["IVCV"]["IV"]["Complience"]):  # Is a dynamic waiting time for the measuremnt
                self.stop_everything()
            ...
            for i in range(samples):
                ...
                values.append(str(self.vcw.query(device_dict, device_dict["get_read"])).split("\t")) # 2657SMU
    ...
    def do_CV(self, voltage, device_dict, samples = 5):
        ...
        if not self.main.event_loop.stop_all_measurements_query():
            ...
            if not self.switching.switch_to_measurement("CV"):
                self.stop_everything()
                return
            if not self.steady_state_check(device_dict, "get_read", max_slope=1e-6, wait=0.05, samples=3, Rsq=0.5, complience=None):  # Is a dynamic waiting time for the measuremnts
                self.stop_everything()
                self.log.warning("Steady state could not be reached, shutdown of measurement")
                return
            values = []
            for i in range(samples):
                values.append(float(str(self.vcw.query(device_dict, device_dict["get_read"])).split(",")[0]))
