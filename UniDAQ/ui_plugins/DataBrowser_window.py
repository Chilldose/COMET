...
class DataBrowser_window:
    ...
    def __init__(self, GUI_classes, layout):
        ...
        self.data_ui = self.variables.load_QtUi_file("Data_explorer.ui", self.data_browser_widget)
        ...
        self.data_ui.key_value_tree.itemClicked.connect(self.import_clicked_value_devices)
        self.data_ui.Change_value_button.clicked.connect(self.change_value_devices_button)
        ...
        self.data_ui.key_value_tree_2.itemClicked.connect(self.import_clicked_value_settings)
        ...
        self.data_ui.remove_item_settings.clicked.connect(self.remove_item_settings_button)
    ...
    def load_device_values(self, item, kwargs=None):
        ...
        try:
            try:
                ...
                for i, keys in enumerate(self.variables.devices_dict[str(
                        self.variables.default_values_dict["settings"]["current_selected_browser_value"])].keys()):
                    ...
                    self.data_ui.key_value_tree.topLevelItem(i).setText(1, self._translate("data_browser", str(
                        self.variables.devices_dict[item][keys])))
            except:
                pass
    ...
    def change_value_devices_button(self, kwargs=None):
        ...
        if self.data_ui.key_edit.text() and self.variables.default_values_dict["settings"][
            ...
            if settings in self.variables.devices_dict and self.data_ui.key_edit.text() in self.variables.devices_dict[
                settings]:  # Has to be so, otherwise mismatch can happen
                try:
                    ...
                    msg.about(None, "Error", "Could not interpret input \"" + str(
                        self.data_ui.value_edit.text()) + "\" \n it seems that this is not a valid literal.")
                    self.log.error("Could not interpret input " + str(
                        self.data_ui.value_edit.text()) + " it seems that this is not a valid literal.")
    ...
    def remove_item_device_button(self, kwargs=None):
        if self.data_ui.key_edit.text() in self.variables.devices_dict[
            self.variables.default_values_dict["settings"]["current_selected_browser_value"]]:
            reply = QMessageBox.question(None, 'Warning', "Are you sure to remove the selected item?", QMessageBox.Yes,
                                         QMessageBox.No)
            if reply == QMessageBox.Yes:
                self.variables.devices_dict[
                    self.variables.default_values_dict["settings"]["current_selected_browser_value"]].pop(
                    self.data_ui.key_edit.text(), None)
                self.reload_devices_button_action()
        else:
            QMessageBox.question(None, 'Warning',
    ...
    def change_value_settings_button(self, kwargs=None):
        try:
            if self.data_ui.key_edit_2.text() and self.variables.default_values_dict["settings"][
                ...
                if settings in self.variables.default_values_dict and self.data_ui.key_edit_2.text() in \
                        self.variables.default_values_dict[settings]:  # Has to be so, otherwise mismatch can happen
                    try:
                        ...
                        msg.setIcon(QMessageBox.Critical)
                        msg.about(None, "Error", "Could not interpret input \"" + str(
                            self.data_ui.value_edit_2.text()) + "\" \n it seems that this is not a valid literal.")
                        self.log.error("Could not interpret input " + str(
                            self.data_ui.value_edit_2.text()) + " it seems that this is not a valid literal.")
    ...
    def remove_item_settings_button(self, kwargs=None):
        try:
            if self.data_ui.key_edit_2.text() in self.variables.default_values_dict[
                self.variables.default_values_dict["settings"]["current_selected_browser_value"]]:
                reply = QMessageBox.question(None, 'Warning', "Are you sure to remove the selected item?",
                                             QMessageBox.Yes, QMessageBox.No)
                if reply == QMessageBox.Yes:
                    self.variables.default_values_dict[
                        self.variables.default_values_dict["settings"]["current_selected_browser_value"]].pop(
                        self.data_ui.key_edit_2.text(), None)
                    self.reload_settings_button_action()
            else:
                QMessageBox.question(None, 'Warning',
                                     "Hold on there Pirate!!! You try to delete an element which does not exist.",
